...
void location_keep_control(powerconpensation_t *powerconpensation)
{
    ...
    powerconpensation->Y.speech_value_now = powerconpensation->Y.speech_value_last + rovInfo.jy901.acc.y;                        //计算速度
    powerconpensation->Y.displacement = (powerconpensation->Y.speech_value_now + powerconpensation->Y.speech_value_last) * 0.5f; //计算位移
    ...
    if (powerconpensation->X.power_conpensation > 50)
